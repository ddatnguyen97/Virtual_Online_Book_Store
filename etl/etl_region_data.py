...
def transform_data(df):
    try:
        df['region_id'] = df['region _id'].astype(str)
...
def insert_on_conflict_nothing(table, conn, keys, data_iter):
    try:
        ...
        stmt = insert(table.table).values(data).on_conflict_do_nothing(index_elements=["region_id"])
...
if __name__ == "__main__":
    ...
    data_file = 'data_source\Virtual Book Store Project - Region List.csv'
    df = get_data(data_file)

    table_name = 'dim_region'
    load_data_to_db(df, table_name, connection_string)
    logging.info("ETL process for region data completed.")