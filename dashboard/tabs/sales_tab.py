...
def sales_tab(selected_date, connection_string  ):
    ...
    with st.sidebar:
        st.header("Filters")
        filters = build_filters(curr_sales_df, columns)
    ...
    total_revenue_metric = create_data_metric("Total Revenue",
                                curr_total_revenue,
                                prev_total_revenue)
    
    total_orders_metric = create_data_metric("Total Orders",
                                curr_total_orders,
                                prev_total_orders)
    
    avg_order_metric = create_data_metric("Avg Order Value",
    ...
    with st.container():
        ...
        with col1:
            st.plotly_chart(total_revenue_metric)

        with col3:
            st.plotly_chart(total_orders_metric)

        with col5:
            st.plotly_chart(avg_order_metric)
    ...
    with st.container():
        ...
        st.plotly_chart(sales_by_date_chart)
    ...
    with st.container():
        ...
        with col2:
            ...
            st.plotly_chart(payment_type_chart)
    ...
    with st.container():
        ...
        with col2:
            ...
            st.plotly_chart(revenue_by_region_chart)